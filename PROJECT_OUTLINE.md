...
*   **Modeling:** XGBoost, CatBoost, Scikit-learn (Joblib for persistence)
...
*   **Recent Changes:** H2O AutoML and FLAML were cleanly removed from the UI and dependencies to simplify the modeling pipeline, strictly favoring XGBoost and CatBoost.
...
### 2.4 Modeling & Tuning (`train.py`, `optuna_search.py`, `src/model.py`, `src/autotune.py`)
